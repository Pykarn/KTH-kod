...
This script always checks multiple sources and combines everything it
finds -- it never stops early just because one source worked:

1. The course's "Files" tab (if the professor left it enabled).
2. The Front Page, Modules, Pages, Assignments, Announcements, and the
   Syllabus -- scanned every time (not just as a fallback) for:
     ...
     b. GitHub repo links (e.g. a professor linking course code/materials
        ...
        entire repo's contents are downloaded automatically, IN ADDITION
        to whatever else is found on Canvas -- one doesn't replace the
        other.
...
def collect_text_blobs(course_id):
    """Gather text/HTML from the Front Page, Modules, Pages, Syllabus,
    Assignments, and Announcements -- basically everywhere a professor
    might drop a link or file."""
    blobs = []
    ...
    front_page, _ = api_get(f"{CANVAS_URL}/api/v1/courses/{course_id}/front_page")
    if front_page and front_page.get("body"):
        blobs.append(front_page["body"])
...
def scan_everywhere(course_id, dest_folder):
    """Always scan the Front Page, Modules, Pages, Assignments, Announcements,
    and Syllabus for GitHub links and Canvas file links -- runs every time,
    even if the Files tab already gave us files, so nothing gets missed."""
    print("  Scanning front page/modules/pages/assignments for extra links...")
    ...
    if not got_any:
        print("  No extra files or GitHub links found here.")
    return got_any
...
def main():
    ...
    for course_name, course_id in COURSE_IDS.items():
        ...
        got_from_files = try_files_api(course_id, dest_folder)
        got_from_scan = scan_everywhere(course_id, dest_folder)
        if not got_from_files and not got_from_scan:
            print("  Nothing downloadable found anywhere for this course.")
