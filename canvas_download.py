...
CANVAS_FILE_LINK_RE = re.compile(r'https?://[^\s"\'<>]*/files/\d+[^\s"\'<>]*')
...
def extract_links(blobs):
    github_links = set()
    canvas_file_links = set()
    for blob in blobs:
        for link in GITHUB_LINK_RE.findall(blob):
            github_links.add(link.rstrip("/"))
        for link in CANVAS_FILE_LINK_RE.findall(blob):
            canvas_file_links.add(link)
    return github_links, canvas_file_links
...
def try_fallback_scan(course_id, dest_folder):
    ...
    github_links, canvas_file_links = extract_links(blobs)
    ...
    for link in canvas_file_links:
        try:
            resp = requests.get(link, headers=HEADERS)
        except requests.RequestException:
            continue
        if resp.ok:
            filename = link.split("/")[-1].split("?")[0] or "file"
            os.makedirs(dest_folder, exist_ok=True)
            with open(os.path.join(dest_folder, filename), "wb") as f:
                f.write(resp.content)
            print(f"  downloaded linked file: {filename}")
            got_any = True
