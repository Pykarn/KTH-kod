...
class Varodatabas:
    ...
    def __str__(self):
        return f'{self.namn:<{stränglängd+2}}{self.antal_skannade:<{stränglängd+2}}{self.pris:<{stränglängd+2}.2f}{self.pris*self.antal_skannade:.2f}'
...
def kvitto(skannade_obj_lista):
    ...
    sträcklängd = 3*(stränglängd+2)+len(f'{total_pris:.2f}')

    #skriver ut självaste kvittot med korrekt format
    print(f'{"Varunamn":<{stränglängd+2}}{"Antal":<{stränglängd+2}}{"A-pris":<{stränglängd+2}}{"Summa"}')
    print(f"{'—'*sträcklängd}")
    for obj in skannade_obj_lista:
        print(obj)
    print(f"{'='*sträcklängd}")
    print(f'{"Total":<{stränglängd+2}}{total_antal:<{2*(stränglängd+2)}}{total_pris:.2f}')
