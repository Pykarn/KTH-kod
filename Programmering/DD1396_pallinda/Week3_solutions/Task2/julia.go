...
func Julia(f ComplexFunc, n int) image.Image {
	...
	//dela upp
	kolonnlängd := bounds.Max.Y - bounds.Min.Y
	antalrader := kolonnlängd / workers

	//dela upp i rader
	for i := 0; i < workers; i++ {
		Ystart := bounds.Min.Y + i*antalrader
		Yend := Ystart + antalrader

		//om length inte är delbar med antal workers
		//så får sista worker ta resten som blir över
		if i == workers-1 {
			Yend = bounds.Max.Y
		}
		//en goroutine för varje grupp av rader
		go func(Ystart, Yend int) {
			defer wg.Done()
			//hela raden från Xmin till Xmax
			for i := bounds.Min.X; i < bounds.Max.X; i++ {
				//bara några rader valda
				//från Ystart till Yend
	...
	//vänta tills alla goroutiner är klara
	wg.Wait()
...
//time go run julia.go, INNAN ändringar:
//go run julia.go  15.52s user 0.28s system 98% cpu 16.076 total
//time go run julia.go, EFTER ändringar:
//go run julia.go  12.98s user 0.18s system 719% cpu 1.829 total
